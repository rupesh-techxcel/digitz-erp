...
def get_data(filters):
	...
	if filters.get('customer') and  filters.get('from_date') and filters.get('to_date'):
		...
		data = frappe.db.sql(""" SELECT si.customer, si.name as sales_invoice_name,si.posting_date as posting_date,si.rounded_total as amount,si.paid_amount,si.rounded_total - IFNULL(si.paid_amount,0) as balance_amount,si.delivery_note as delivery_note,si.ship_to_location as ship_to_location FROM `tabSales Invoice` si where si.docstatus = 1 and si.credit_sale=1  and si.posting_date BETWEEN '{0}' and '{1}' and ('{2}'.lower()=='true' or  paid_amount< rounded_total) order by si.posting_date""".format(filters.get('from_date'),filters.get('to_date'), filters.get('show_all_invoices')),as_dict=True)
	elif filters.get('customer'):
		data = frappe.db.sql(""" SELECT si.name as sales_invoice_name,si.posting_date,si.rounded_total as amount,si.paid_amount,si.rounded_total - IFNULL(si.paid_amount,0) as balance_amount,si.delivery_note as delivery_note,si.ship_to_location as ship_to_location FROM `tabSales Invoice` si where si.docstatus = 1 and si.credit_sale=1 and si.customer = '{0}'  and ('{1}'.lower()=='true' or paid_amount< rounded_total)  order by posting_date""".format(filters.get('customer'),format(filters.get('show_all_invoices'))),as_dict=True)

	else:
		data = frappe.db.sql(""" SELECT si.customer,si.name as sales_invoice_name,si.posting_date as posting_date,si.rounded_total as amount,si.paid_amount,si.rounded_total - IFNULL(si.paid_amount,0) as balance_amount,si.delivery_note as delivery_note,si.ship_to_location as ship_to_location FROM `tabSales Invoice` si where si.docstatus = 1 and si.credit_sale=1  and ('{0}'.lower()=='true' or paid_amount< rounded_total) order by posting_date""".format(filters.get('show_all_invoices')),as_dict=True)
